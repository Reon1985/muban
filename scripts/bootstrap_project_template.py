...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bootstrap the project-doc-ops template into a target repository."
    )
    parser.add_argument("--target", required=True, help="Absolute path to the target repository")
    parser.add_argument(
        "--project-name",
        default="your-project",
        help="Project name used in template placeholders",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing files when collisions happen",
    )
...
def main() -> None:
    ...
    template_root = repo_root / "templates" / "project-doc-ops-template"
